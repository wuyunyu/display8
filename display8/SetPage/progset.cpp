...
//程序管理--刷新程序名称和行数
void ProgSet::Program_Display_Refresh(u8 flag)
{
    if(flag)
    {
        for(int i=0; i<SAVEPROGRAMNUM; i++)
        {
            GP->Parameter_StringChang(GP->Temp_Display_Data, 0, GP->Free_Program_Save[i].Name,
                                      GP->Free_Program_Save[i].Name2, GP->Free_Program_Save[i].Name3);
            Label_ProgN_List.at(i)->setText(GP->u8toqstr(GP->Temp_Display_Data, 12));
            Label_ProgS_List.at(i)->setText(QString::number(static_cast<int>(GP->Free_Program_Save[i].Num)));
        }
        flag = FALSE;
    }
}
